...
plot.background = element_rect(fill = "#fff6ec"), text = element_text(family = "Verdana"), legend.position = "bottom", axis.title.x = element_blank(), axis.text = element_text(size = 20),
    ...
    axis.title.y = element_text(size = 20))) #Setting the ggplot theme

owid_energy <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-06-06/owid-energy.csv')
owid_energy_fr <- owid_energy |>    
    filter(country == "France") #Filtering data on France only
owid_energy_fr_prod <- owid_energy_fr |>
    select(year, ends_with("share_elec")) |> #Removing unused columns
    select(-c(fossil_share_elec, renewables_share_elec, other_renewables_share_elec, low_carbon_share_elec)) |> #Removing unused columns
    pivot_longer(cols = !year, names_to = "Energy_type", values_to = "share") |> #Pivot to long format
    mutate(Energy_type = str_to_title(gsub("_share_elec", "", Energy_type))) #Simplification and capitalization of column names
