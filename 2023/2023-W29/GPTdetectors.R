...
ggplot(det_sum) +
geom_waffle(aes(values = pct, fill = native), na.rm = FALSE, flip = TRUE, radius = unit(20, "pt"), size = 0) +
...
text = element_text(color = "white", family = "vd"),
panel.grid.minor = element_blank(), legend.background = element_rect(fill = "#25251f", color = NA),
...
plot.caption = element_markdown(color = "white", margin = margin(t = 30), size = 8, hjust = 1),
